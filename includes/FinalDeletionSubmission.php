...
class FinalDeletionSubmission {
    ...
    private function delete_files( $post_data ) {
        ...
        if ( $verification !== 'DELETE' ) {
            $this->alert( __( 'Confirmation does not match. Files not deleted.', 'buddyclients-lite' ) );
            return;
        }
    ...
    private function generate_alert_message( $successful_deletions, $failed_deletions ) {
        ...
        if ( ! empty( $successful_deletions ) ) {
            $alert_message .= sprintf(
                ...
                _n( '%d file successfully deleted.', '%d files successfully deleted.', count( $successful_deletions ), 'buddyclients-lite' ),
        ...
        if ( ! empty( $failed_deletions ) ) {
            $alert_message .= sprintf(
                ...
                _n( '%d file was not deleted. Please try again.', '%d files were not deleted. Please try again.', count( $failed_deletions ), 'buddyclients-lite' ),
