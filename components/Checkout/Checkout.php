...
class Checkout {
    ...
    private function test_mode( $tag_only = false ) {
        if ( class_exists( StripeKeys::class ) ) {
            ...
            if ( $mode === 'test' ) {
                $content = '<div><p class="bc-test-mode-tag">' . __( 'Test Mode', 'buddyclients-free' ) . '</p></div>';
                if ( ! $tag_only ) {
                    $content .= '<p class="bc-test-instructions">' . __( 'Use card number 4242 4242 4242 4242 to simulate a successful payment.', 'buddyclients-free' ) . '</p>';
                }
                return $content;
            }
        }
    ...
    private function policies() {
        ...
        $privacy_policy = bc_get_setting( 'pages', __( 'privacy_policy', 'buddyclients-free' ) );
        $site_terms = bc_get_setting( 'pages', __( 'terms_of_service', 'buddyclients-free' ) );
    ...
    private function stripe_form() {
        if ( class_exists( StripeForm::class ) ) {
            ...
            $message = '<p>' . __( 'Payments are not enabled on this website. ', 'buddyclients-free' ) . bc_contact_message() . '</p>';
