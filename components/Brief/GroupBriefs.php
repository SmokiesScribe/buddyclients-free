...
class GroupBriefs {
    ...
    public function build() {
        ...
        if ( $this->briefs && ! empty ( $this->briefs ) ) {
            ...
            $content .= __( 'No briefs available.', 'buddyclients-lite' );
        }
    ...
    private function brief_message( $complete ) {
        $click_action = $complete ? __( 'view', 'buddyclients-lite' ) : __( 'complete', 'buddyclients-lite' );
        return sprintf(
            /* translators: %s: the click action (e.g. view) */
            __( 'Click to %s.', 'buddyclients-lite' ),
            $click_action
        );
    }
