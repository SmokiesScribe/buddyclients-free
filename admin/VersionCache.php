...
class VersionCache {
    ...
    private function __construct() {
        if ( ! defined( 'BUDDYC_PLUGIN_VERSION' ) ) return;
        $this->get_versions();
    ...
    private function get_versions() {
        $this->curr_version = get_option( 'buddyc_curr_version' );
        $this->prev_version = get_option( 'buddyc_prev_version' );
    }
    ...
    private function update_versions() {
        ...
        if ( BUDDYC_PLUGIN_VERSION !== $this->curr_version ) {
            ...
            // Update options
            update_option( 'buddyc_prev_version', $old_version );
            update_option( 'buddyc_curr_version', $new_version );
             ...
             */
            do_action( 'buddyc_version_updated', $old_version, $new_version );
        }
    }
}