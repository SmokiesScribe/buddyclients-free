...
use BuddyClients\Admin\SettingsRouter;
...
class Settings {
    ...
    private static function cache_key( $settings_group, $version = null ) {
        ...
        return '_buddyc_settings_cache_' . $settings_group . '_' . $formatted_version;
    }
    ...
    private function not_processed( $post_id, $hook ) {
        ...
        if ( $hook === 'before_delete_post' || $hook === 'wp_trash_post' ) {
            ...
            // Check if already processed
            $last_processed = get_transient( 'buddyc_last_trashed_page' );
            if ( $last_processed == $post_id ) return false;

            // Set transient to prevent repeated execution
            set_transient( 'buddyc_last_trashed_page', $post_id, 30 ); // Expires in 30 seconds
        }
    ...
    public function clear_cache_post_types() {
        ...
        $cached_post_types = get_option( '_buddyc_cached_post_types', [] );
        ...
        if ( $post_types !== $cached_post_types ) {
            ...
            // Update cache
            update_option( '_buddyc_cached_post_types', $post_types );
        }
    }
   ...
   /**
    ...
    private static function settings_group_name( $settings_group ) {
        return 'buddyc_' . $settings_group . '_settings';
    }
}